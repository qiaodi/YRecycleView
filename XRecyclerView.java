...
public class XRecyclerView extends RecyclerView {
    ...
    private static final float DRAG_RATE = 2; // 手指移动距离和view移动距离的比值，2表示手指滑动一屏，刷新view到半屏
    ...
    public void setRefreshHeader(BaseRefreshHeader refreshHeader) {
        if (pullRefreshEnabled) {
            mHeaderViews.add(0, (View) refreshHeader);
            mRefreshHeader = refreshHeader;
        }
    }
...
//            Logger.d(TAG,"onScrollStateChanged layoutManager.getChildCount() "+layoutManager.getChildCount()+" lastVisibleItemPosition "+ lastVisibleItemPosition + " layoutManager.getItemCount() "+layoutManager.getItemCount());
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                mLastY = ev.getRawY();
                if (isOnTop() && pullRefreshEnabled && mRefreshHeader != null) {
                    mRefreshHeader.onMove(deltaY / DRAG_RATE);
                    if (mRefreshHeader.getVisibleHeight() > 0 && mRefreshHeader.getState() < BaseRefreshHeader.STATE_REFRESHING) {
                        Logger.d(TAG, "getVisibleHeight = " + mRefreshHeader.getVisibleHeight());
                        return false;
                    }
                }
                break;
    ...
    public void setRefreshing(boolean refreshing) {
        if (refreshing && pullRefreshEnabled && mLoadingListener != null && mRefreshHeader != null) {
            mRefreshHeader.onMove(mRefreshHeader.getRefreshingHeight());
